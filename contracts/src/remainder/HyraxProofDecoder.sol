...
library HyraxProofDecoder {
    ...
    /// @notice Decode a PODP proof from ABI-encoded bytes at the given offset
    /// @return podp The decoded PODP proof
    /// @return newOffset The offset after the PODP data
    function decodePODP(bytes calldata data, uint256 offset)
        internal
        pure
        returns (HyraxVerifier.PODPProof memory podp, uint256 newOffset)
    {
        // commitD (G1 point: x, y)
        podp.commitD.x = _readU256(data, offset);
        podp.commitD.y = _readU256(data, offset + 32);
        offset += 64;

        // commitDDotA (G1 point: x, y)
        podp.commitDDotA.x = _readU256(data, offset);
        podp.commitDDotA.y = _readU256(data, offset + 32);
        offset += 64;

        // z_vector: length-prefixed array of Fr scalars
        uint256 numZ = _readU256(data, offset);
        offset += 32;
        podp.zVector = new uint256[](numZ);
        for (uint256 i = 0; i < numZ; i++) {
            podp.zVector[i] = _readU256(data, offset);
            offset += 32;
        }

        // z_delta
        podp.zDelta = _readU256(data, offset);
        offset += 32;

        // z_beta
        podp.zBeta = _readU256(data, offset);
        offset += 32;

        newOffset = offset;
    }

    /// @notice Decode full input proofs with PODP data (commitment rows + PODP proofs + comEval)
    /// @return evalProofs Array of HyraxVerifier.EvalProof structs
    function decodeInputProofsWithPODP(bytes calldata data)
        internal
        pure
        returns (HyraxVerifier.EvalProof[] memory evalProofs)
    {
        uint256 offset = _offsetToInputProofs(data);
        uint256 numInputProofs = _readU256(data, offset);
        offset += 32;

        evalProofs = new HyraxVerifier.EvalProof[](numInputProofs);

        for (uint256 i = 0; i < numInputProofs; i++) {
            // Commitment rows
            uint256 numCommits = _readU256(data, offset);
            offset += 32;
            evalProofs[i].commitmentRows = new HyraxVerifier.G1Point[](numCommits);
            for (uint256 j = 0; j < numCommits; j++) {
                evalProofs[i].commitmentRows[j].x = _readU256(data, offset);
                evalProofs[i].commitmentRows[j].y = _readU256(data, offset + 32);
                offset += 64;
            }

            // Evaluation proofs (PODP + commitmentToEvaluation per eval)
            uint256 numEvals = _readU256(data, offset);
            offset += 32;

            // For the common case: one eval proof per input proof
            // We store the first PODP + comEval into the EvalProof
            for (uint256 j = 0; j < numEvals; j++) {
                if (j == 0) {
                    (evalProofs[i].podp, offset) = decodePODP(data, offset);
                    // commitmentToEvaluation
                    evalProofs[i].comEval.x = _readU256(data, offset);
                    evalProofs[i].comEval.y = _readU256(data, offset + 32);
                    offset += 64;
                } else {
