...
# =============================================================================
# Stylus Dispute E2E Test (TEEMLVerifier Full Lifecycle)
# =============================================================================
#
# Runs a full TEEMLVerifier dispute lifecycle on a local Arbitrum nitro-devnode
# using the Stylus (WASM) verification path:
#
#   1. Start nitro-devnode (Docker)
#   2. Deploy Stylus GKR verifier (cargo stylus deploy)
#   3. Run Forge script that:
#      - Deploys RemainderVerifier + registers DAG circuit + sets Stylus verifier
#      - Deploys TEEMLVerifier + enables Stylus routing
#      - Registers enclave + submits TEE result (EIP-712 signed)
#      - Challenges the result (from separate account)
#      - Resolves dispute via Stylus verification (real proof data)
#      - Verifies: disputeResolved=true, disputeProverWon=true
#
# Prerequisites:
#   - Docker running
...
#
# Usage:
#   ./scripts/stylus-dispute-e2e.sh [--skip-devnode] [--skip-deploy] [--contract ADDRESS]
...
echo "============================================="
echo "  Stylus Dispute E2E Test"
echo "  (TEEMLVerifier Full Lifecycle)"
echo "============================================="
echo ""
...
CONTAINER_NAME="nitro-stylus-dispute-e2e"
...
# =============================================================================
# Phase 3-7: Run Forge E2E script (deploy + register + submit + challenge + resolve)
# =============================================================================

echo "[Phases 3-7] Running Forge dispute E2E script..."
echo ""
...
echo "============================================="
echo "  Dispute E2E Results Summary"
echo "============================================="
echo ""
echo "  Stylus contract:    $CONTRACT_ADDRESS"

# Extract gas info from Forge output if available
RESOLVE_GAS=$(echo "$FORGE_OUTPUT" | grep -i "Gas used" | grep -oE '[0-9]+' | head -1 || true)
if [ -n "$RESOLVE_GAS" ]; then
    RESOLVE_M=$(echo "scale=1; $RESOLVE_GAS / 1000000" | bc 2>/dev/null || echo "N/A")
    echo "  Resolve gas (approx): $RESOLVE_GAS (${RESOLVE_M}M)"
fi

echo ""

# Check for success
if echo "$FORGE_OUTPUT" | grep -q "STYLUS DISPUTE E2E PASSED"; then
    echo "=== STYLUS DISPUTE E2E PASSED ==="
else
    echo "=== STYLUS DISPUTE E2E FAILED ==="
    exit 1
fi
