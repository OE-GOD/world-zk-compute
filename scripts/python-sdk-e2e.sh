...
# ═══════════════════════════════════════════════════════════════════════════════
# Python SDK BatchVerifier E2E Test
#
# End-to-end test for the Python BatchVerifier SDK:
# 1. Start Anvil with code-size-limit disabled
# 2. Deploy RemainderVerifier + register DAG circuit via Forge
# 3. Run Python BatchVerifier SDK against the deployed contract
# ═══════════════════════════════════════════════════════════════════════════════
...
cd "$SDK_DIR"
pip install -e ".[web3]" --quiet
ok "Python SDK installed"

# ── Step 4: Run Python BatchVerifier ─────────────────────────────────────────
log "Step 4: Running Python BatchVerifier SDK..."

OUTPUT=$(python e2e_batch_verify.py \
    --rpc-url "$RPC_URL" \
    --private-key "$DEPLOYER_KEY" \
    --contract "$CONTRACT_ADDR" \
    --fixture "$FIXTURE_PATH" 2>&1) || {
    err "Python BatchVerifier SDK failed"
    echo "$OUTPUT"
    exit 1
}
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  Python SDK BatchVerifier E2E Test PASSED"
echo "════════════════════════════════════════════════════════════"
